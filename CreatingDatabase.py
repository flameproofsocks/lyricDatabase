...
artist = api.search_artist(artistName, max_songs=100) #find all the songs. #can be changed to whatever artist.
...
songTitleList = songTitleList.replace("]", "")
songTitleList = songTitleList.replace(", 'Ski Mask the Slump God'), (", "^")
songTitleList = songTitleList.replace(", 'Ski Mask the Slump God')", "")
songTitleList = songTitleList.replace("(", "")
songTitleList = songTitleList.replace("'", "")
...
file1 = open("Lyrics_SkiMasktheSlumpGod.txt","r", encoding="utf8") #read-only open the file
